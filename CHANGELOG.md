...
- With the display-server crash fixed, CI's e2e run got further but still failed: `context.addCookies()` was seeding the "logged into Etsy" session cookie before any page had ever navigated to an `etsy.com` origin, and `ADD_ACCOUNT`'s `chrome.cookies.getAll` call wasn't seeing it. Added `e2e/etsy-session-helper.ts`, used by all three specs, which navigates a route-mocked `https://www.etsy.com/` page first and *then* seeds the cookie — closer to how a real logged-in browser actually got that cookie in the first place.
